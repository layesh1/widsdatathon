...
# ── Live incident feed (replaces raw fire_data_integration calls) ─────
from live_incident_feed import (
    get_live_incidents,
    get_incident_summary,
    get_escalating_fires,
    render_feed_status_badge,
)
from fire_data_integration import find_nearby_fires
...
def load_fire_data():
    """
    Load fire incident data via the live_incident_feed priority chain:
      1. geo_events_geoevent.csv (WiDS dataset — is_active fires)
      2. NASA FIRMS VIIRS hotspots (fallback)
      3. Demo data (offline fallback)
    Returns a standardized DataFrame.
    """
    return get_live_incidents(active_only=True, days_back=30)
...
def render_command_dashboard(fire_data, vulnerable_populations, usfa_data):
    ...
    # Use standardized summary from live_incident_feed
    summary = get_incident_summary(fire_data)
    insights = load_real_insights()

    # Live fire metrics — now from live_incident_feed summary
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Active Fires",          summary["total_active"])
    c2.metric("Evacuation Orders",     summary["evacuation_orders"])
    c3.metric("Warnings Issued",       summary["evac_warnings"])
    c4.metric("Counties Monitored",    len(vulnerable_populations))

    # Data source indicator
    source_labels = {
        "geo_events": "🟢 WiDS Dataset (geo_events)",
        "firms":      "🟡 NASA FIRMS VIIRS",
        "demo":       "🔴 Demo data",
    }
    st.caption(
        f"Incident feed: **{source_labels.get(summary['source'], summary['source'])}**  ·  "
        f"Updated: {summary['last_updated']}  ·  "
        f"High-confidence: {summary['high_confidence_pct']}%"
    )
    ...
    with col_map:
        ...
        if GEO_MAP_AVAILABLE:
            ...
            if len(fire_data) > 0:
                # fire_data now uses standardized columns: lat, lon, name, acreage
                df = fire_data.copy()
                if 'acreage' in df.columns:
                    df = df.nlargest(100, 'acreage')
                else:
                    df = df.head(100)
                for _, fire in df.iterrows():
                    acres = fire.get('acreage', 0) or 0
                    # Color by notification type
                    nt = str(fire.get('notification_type', '')).lower()
                    color = 'red' if 'order' in nt else 'orange' if 'warning' in nt else 'lightred'
                    folium.Circle(
                        location=[fire['lat'], fire['lon']],
                        radius=min(max(acres * 50, 20000), 100000),
                        color=color, fill=True, fillColor=color, fillOpacity=0.4,
                        popup=f"<b>{fire.get('name','Unknown')}</b><br>"
                              f"{acres:,.0f} acres<br>{fire.get('notification_type','')}"
                    ).add_to(m)
    ...
    with col_alerts:
        ...
        # Use only escalating fires (orders + warnings) for triage
        escalating = get_escalating_fires(fire_data)

        if len(fire_data) > 0:
            # Build find_nearby_fires-compatible format from standardized df
            fire_compat = fire_data.rename(columns={
                'lat': 'latitude', 'lon': 'longitude', 'name': 'fire_name', 'acreage': 'acres'
            })
            alerts = find_nearby_fires(fire_compat, vulnerable_populations, radius_km=80)
            if alerts:
                ...
                if len(escalating) > 0:
                    st.error(f"🚨 {len(escalating)} fires with Orders/Warnings")
...
def render_start_here(fire_data, vulnerable_populations):
    ...
    # Show active evacuation orders prominently if any
    if summary["evacuation_orders"] > 0:
        st.error(f"🚨 **{summary['evacuation_orders']} active evacuation orders** in the current incident feed.")
    if summary["evac_warnings"] > 0:
        st.warning(f"⚠️ **{summary['evac_warnings']} evacuation warnings** currently issued.")
    ...
    # Use escalating fires (orders/warnings) for caregiver view — highest urgency
    escalating = get_escalating_fires(fire_data)
    display_fires = escalating if len(escalating) > 0 else fire_data

    if len(display_fires) > 0:
        for _, fire in display_fires.head(10).iterrows():
            name  = fire.get('name', 'Unknown Fire')
            acres = fire.get('acreage', 0) or 0
            nt    = fire.get('notification_type', '')
            icon  = "🚨" if 'order' in str(nt).lower() else "⚠️" if 'warning' in str(nt).lower() else "🔥"
            label = f" — **{nt}**" if nt else ""
            st.warning(f"{icon} **{name}** — {acres:,.0f} acres{label}")
    else:
...
def render_analyst_dashboard(fire_data, vulnerable_populations, wids_data):
    ...
    # Top metrics — live feed + real data
    c1, c2, c3, c4 = st.columns(4)
    if insights:
        ...
        c1.metric("Active Fires (Feed)",   summary["total_active"])
        c2.metric("Evac Orders",           summary["evacuation_orders"])
        c3.metric("Warnings",              summary["evac_warnings"])
        c4.metric("Counties Monitored",    len(vulnerable_populations))

    # Feed source badge for analysts
    source_labels = {
        "geo_events": "🟢 WiDS geo_events dataset",
        "firms":      "🟡 NASA FIRMS VIIRS",
        "demo":       "🔴 Demo data (offline)",
    }
    st.caption(
        f"Incident feed: **{source_labels.get(summary['source'], summary['source'])}**  ·  "
        f"{summary['total_active']} active fires  ·  "
        f"High-confidence: {summary['high_confidence_pct']}%  ·  "
        f"Updated: {summary['last_updated']}"
    )
    ...
    with col1:
        ...
        # Also plot fires from the live feed
        if len(fire_data) > 0:
            for _, fire in fire_data.head(50).iterrows():
                nt = str(fire.get('notification_type', '')).lower()
                color = 'red' if 'order' in nt else 'orange' if 'warning' in nt else 'gray'
                folium.CircleMarker(
                    location=[fire['lat'], fire['lon']],
                    radius=5, color=color, fill=True, fillOpacity=0.7,
                    popup=f"<b>{fire.get('name','Fire')}</b><br>{fire.get('notification_type','')}"
                ).add_to(m)
...
def render_sidebar(fire_data):
    with st.sidebar:
        ...
        render_feed_status_badge(fire_data)
        st.divider()
...
def main():
    ...
    with st.spinner("Loading data..."):
        fire_data              = load_fire_data()   # now via live_incident_feed
